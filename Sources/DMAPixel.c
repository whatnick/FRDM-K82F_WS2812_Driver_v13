...
#include "fsl_sim_hal_MK82F25615.h"
...
void DMA_Transfer(uint8_t *transmitBuf, uint32_t nofBytes) {
  ...
  config.destAddr = (uint32_t)&GPIO_PSOR_REG(GPIOD_BASE_PTR); /* Port Set Output register */
  ...
  config.destAddr = (uint32_t)&GPIO_PDOR_REG(GPIOD_BASE_PTR); /* Port Data Output register */
  ...
  config.srcAddr = (uint32_t)&OneValue; /* Bit set */
  config.destAddr = (uint32_t)&GPIO_PCOR_REG(GPIOD_BASE_PTR); /* Port Clear Output register */
  config.srcOffset = 0; /* do not increment source address */
...
static void InitHardware(void) {
  ...
  GPIO_PDDR_REG(GPIOD_BASE_PTR) |= (1<<0); /* PTD0 as output */
